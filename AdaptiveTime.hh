...
class AdaptiveTime {
public:
  AdaptiveTime(): growthratio(sqrt(2.0)), decayratio(4.0) {}

  AdaptiveTime(double grratio, double dcratio) : growthratio(grratio), decayratio(dcratio) {
    std::cerr << growthratio << ":" << decayratio << std::endl;
  }
...
#pragma omp master
        {
          if (statOutput) std::cerr << std::endl << "DeltaT(+)" << ":" << thistime << ":" << j << ":" << deltaT << std::endl;
        }
      }
...
#pragma omp master
      {
        if (statOutput) std::cerr << "rollback_step2:" << std::flush;
      }
#pragma omp for
      for (int i=0;i<ndev;++i) {
...
#pragma omp master
      {
        if (statOutput) std::cerr << "done: " << std::flush;
        if (statOutput) std::cerr << std::endl << "DeltaT(-)" << ":" << thistime << ":" << j << ":" << deltaT << std::endl;
      }
    }
    return j;
...
#pragma omp master
    {
      if (statOutput) std::cerr << "DeltaT(0)" << ":" << thistime << ":" << j << ":" << deltaT << std::endl;
    }
  }
