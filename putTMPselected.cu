...
void putTMPselected::setupSelectedTMP(uint32_t s1, uint32_t s2, uint32_t u1, uint32_t u2) {
  ...
  selected[0] = s1; selected[1] = s2;
  unselected[0] = u1; unselected[1] = u2;
}
...
void putTMPselected::putUnSelected(const char *filename) {
  ...
  cudaMemcpy(&(TMP[0]), &(r[unselected[0]]), sizeof(float4) * unselected[1], cudaMemcpyDeviceToHost);
  ...
  ofil << "ITEM: TIMESTEP" << std::endl << 0 << std::endl
       << "ITEM: NUMBER OF ATOMS" << std::endl << N << std::endl
