...
void cudaParticleSPHBase::setup(int n) {
  ...
  if (withInfo) ErrorInfo("malloc n[] on GPU");
  ...
  if (withInfo) ErrorInfo("malloc rhoinv[] on GPU");
...
void cudaParticleSPHBase::calcKernels(void) {
  ...
  if (withInfo) ErrorInfo("calc Kernel for SPH");
#endif
}
...
void cudaParticleSPHBase::calcDensity(void) {
  ...
  assert(_thnum.x*_thnum.y <= threadsMax);
  _mpnum.x = totalNumBlock; _mpnum.y = 27; _mpnum.z = 1;
  ...
  calcF_IJpairWithBlock_F4<SPHcalcDensity<SPHKernelLucy> ><<<_mpnum, _thnum>>>(P, r,
    ...
    N);
  if (withInfo) ErrorInfo("calc Density with Cutoff Block (SPHBase)");

  reduce27_F4<<<MPnum, THnum1D>>>(num, tmp81N, N);
#else
  ...
  if (withInfo) ErrorInfo("GEMV by CUBLAS");
#endif
...
void cudaParticleSPHBase::rollback(real dt) {
  cudaParticleLF::rollback(dt);
}
void cudaParticleSPHBase::rollback2(real dt) {
