...
void createInitialState(CUDAenv<cudaParticleDEM> &particles) {
  ...
  real cell[6] = {-delta, ((int)(L1 / lunit) * lunit) + delta, -delta, ((int)(L2 / lunit) * lunit) + delta, 0.0, L3 * static_cast<real>(2.5) + static_cast<real>(6.3) + R0};
    ...
    for (int i=0;i<lsize;++i)
      for (int j=0;j<lsize2;++j) {
        if ((((i<1) || (lsize-2<i) ||
          ...
          if (pb.r[0]<cell[0]) pb.r[0] = cell[0];
          if (pb.r[0]>cell[1]) pb.r[0] = cell[1] - 0.001;
          if (pb.r[1]<cell[2]) pb.r[1] = cell[2];
          if (pb.r[1]>cell[3]) pb.r[1] = cell[3] - 0.001;
          if (k==0)
...
#pragma omp master
    {
      std::cerr << "DeltaT(0)" << ":" << thistime << ":" << 0 << ":" << deltaT << std::endl;
...
#pragma omp for
      for (int i=0;i<ndev;++i) {
        ...
        real _r = 0.0;
        uint32_t _r1 = 0;
        //res[i] = particles[i].inspectVelocity((2*R0) / deltaT, 0.01, 0.01 / 16, _r, _r1, true);
        //std::cerr << "TimeMesh: " << thistime << " " << deltaT << "\t" << _r
        //          << "\t" << _r * ((2 * R0) / deltaT) << "\t" << _r1 << std::endl;
        res[i] = particles[i].inspectVelocity((2 * R0) / deltaT, 0.01, 0.01 / 16, _r, _r1, false);
      }
...
#pragma omp for
          ...
          std::cerr << std::endl << "DeltaT(+)" << ":" << thistime << ":" << j << ":" << deltaT << std::endl;
        }
        if (wait_change_deltaT>0) --wait_change_deltaT;
      } else {
...
#pragma omp master
    {
      particles[0].waitPutTMP();
      std::cerr << "DeltaT(0)" << ":" << thistime << ":" << j << ":" << deltaT << std::endl;
    }
  }
