...
public:
  ...
  void setThnum(T &p)
  {
    int MPnum, THnum1D, THnum2D;
    MPnum = getNumMP();
    THnum1D = getNumCpMP() * 4;
    THnum2D = std::min(getNumCpMP() / ((getCapability() < 3) ? 2 : 4),
                       static_cast<uint32_t>(sqrt(getNumTh())));
    std::cerr << "size of CUDA grid: "
              << MPnum << ", " << THnum1D << ", " << THnum2D << std::endl;
    p.MPnum = MPnum;
    p.THnum1D = THnum1D;
    p.THnum2D = THnum2D;
   ...
   */
  void exchangeForceSelected(const int typeID = 0);
...
CUDAenv<T>::CUDAenv()
{
  ...
  {
    std::cerr << "  Device " << i << ": ";
    t = cudaGetDeviceProperties(&prop, i);
    if (t != 0)
      std::cerr << cudaGetErrorString(t) << std::endl;
...
template <typename T>
void CUDAenv<T>::exchangeForceSelected(const int typeID)
{
  if (ndev > 1)
  {
    if (!enableAllGPUdirect)
