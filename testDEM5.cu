...
#pragma omp parallel for
  for (int i = 0; i < ndev; ++i)
  {
    ...
    particles[i].setDEMProperties(2.11e07, 0.40, 0.29, gamma, 0.10, rad);
...
#pragma omp for
      for (int i = 0; i < ndev; ++i)
      {
        ...
        real _r = 0.0;
        res[i] = particles[i].inspectVelocity((2 * R0) / thistime.currentDeltaT(), 0.01, 0.01 / 16, _r);
        //std::cerr << "TimeMesh: " << thistime() << " " << thistime.currentDeltaT() << "\t" << _r << std::endl;
      }
