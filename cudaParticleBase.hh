...
protected:
  ...
  float4 *r;  //!< pointer to the array on GPU for x[N] y[N] z[N] (float4 x size N)

  float4 *v;  //!< pointer to the array on GPU for vx[N] vy[N] vz[N]

  float4 *a;  //!< pointer to the array on GPU for ax[N] ay[N] az[N]

  real *minv; //!< pointer to the array on GPU for 1/mass [N]

  float4 *F;  //!< pointer to the array on GPU for Fx[N] Fy[N] Fz[N]

  float4 *tmp3N;  //!< pointer to the array on GPU for the temporary area
  ...
  std::set<ArrayType> omitlist;

public:

  cudaParticleBase() :
    r(NULL), minv(NULL), v(NULL), a(NULL), F(NULL), tmp3N(NULL),
   ...
   *
   */
  void treatRefrectCondition(void);
   ...
   *
   * @param vlim  velocity to define the limit range (2R/\Delta t)
   * @param lim_u ratio for the upper limit
   * @param lim_l ratio for the lower limit
   * @param _r  largest *ratio*; largest v / v_limit (output for debug)
   * @param _r1 ID of the fastest particle (output for debug)
   * @param DEBUG if true, obtain all vratio
   * @return  1: vratio>lim_u, 0: lim_u>vratio>lim_l, -1: lim_l>vratio
   */
  int inspectVelocity(real vlim, real lim_u, real lim_l, real &_r, uint32_t &_r1, bool debug=false);
  ...
  int inspectVelocity(real vlim, real lim_u, real lim_l) {
    real _r = 0.0;
    uint32_t _r1 = 0;
    return inspectVelocity(vlim, lim_u, lim_l, _r, _r1, false);
  }
