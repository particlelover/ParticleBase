...
{
protected:
  real *w; //!< pointer to the array on GPU for angular velocity omega_i[N] omega_j[N] omega_k[N]

  real *L; //!< pointer to the array on GPU for angular momentum

  real *T; //!< pointer to the array on GPU for Torque T_i[N] T_j[N] T_k[N]

  real *m0inv; //!< pointer to the array on GPU for 1/(2/5r^2m) [N]

public:
  cudaParticleRotation() : w(NULL), m0inv(NULL), L(NULL), T(NULL){};
