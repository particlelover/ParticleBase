...
protected:
  ...
  // mu the viscosity for field value is placed on num[] as .z
  //real *mu; //!< viscosity for field value (calculated as \mu \rho)
public:
  cudaParticleSPH_NS() : mu0(NULL), c2(NULL), ___p1(BINDEX_SEARCH_WIDTH), ___p2(BINDEX_SEARCH_WIDTH) {
    omitlist.insert(ArrayType::force);
  };
   ...
   * @param typeID  4: num/rho/mu
   */
  void importForceSelected(const cudaParticleSPH_NS &A, const ExchangeMode typeID,
    bool directAccess=false, int idMe=0, int idPeer=0);

private:
  uint32_t offset_additional_output;
